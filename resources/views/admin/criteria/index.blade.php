    ...
    <div class="p-4 mt-20 sm:p-6 lg:p-8">
        ...
        <div class="mb-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 sm:text-3xl">Evaluation Criteria Management</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Manage your evalutaion criterias</p>
                </div>
                <!-- Button Create -->
            ...
            {{-- Modal Create --}}
            @include('admin.criteria.create-modal')
        </div>

        <!-- Table Section -->
        <div class="relative overflow-auto rounded-lg shadow-md">
            @include('admin.criteria.table-criteria')
        </div>
    </div>
@endsection
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#formCreate').on('submit', function(e) {
                const minScore = parseInt(this.min_score_range.value);
                const maxScore = parseInt(this.max_score_range.value);
                ...
                if (!this.checkValidity()) {
                    ...
                    if (!this.quiz_id.value) iziToast.error({
                        title: 'Error',
                        message: 'Nama Quiz wajib diisi',
                    });

                    if (!this.min_score_range.value) iziToast.error({
                        title: 'Error',
                        message: 'Min Skor wajib diisi',
                    });

                    if (!this.max_score_range.value) iziToast.error({
                        title: 'Error',
                        message: 'Max Skor wajib diisi',
                    });
                    ...
                    if (maxScore <= minScore) {
                        iziToast.error({
                            title: 'Error',
                            message: 'Skor maksimum harus lebih besar dari minimum',
                        });
                    }
