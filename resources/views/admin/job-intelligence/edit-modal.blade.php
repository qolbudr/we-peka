<div id="edit-modal_{{ $job->id }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative bg-white rounded-lg shadow-lg">
            ...
            <div class="flex items-center justify-between p-4 border-b border-gray-200 rounded-t md:p-5">
                ...
                <button type="button"
                    ...
                    data-modal-hide="edit-modal_{{ $job->id }}">
            ...
            <!-- Modal body -->
            <form id="formEdit" action="{{ route('job-intelligence.update', $job->id) }}" method="POST">
                @csrf
                ...
                <div class="p-4 space-y-4 md:p-5">
                    <!-- Intelligence Name -->
                    <div id="intelligence-wrapper-{{ $job->id }}">
                        <label for="intelligence-id-{{ $job->id }}"
                            class="block mb-2 text-sm font-medium text-gray-900">
                            Intelligence Name <span class="text-red-500">*</span>
                        </label>
                        <select name="intelligence_id" id="intelligence-id-{{ $job->id }}" required
                            ...
                            @foreach ($intelligences as $intel)
                                <option value="{{ $intel->id }}" class="capitalize"
                                    {{ $intel->id == $job->intelligence_id ? 'selected' : '' }}>
                                    {{ $intel->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="name" id="job-name" value="{{ $job->name }}"
                ...
                <div class="flex items-center justify-end gap-3 p-4 border-t border-gray-200 rounded-b md:p-5">
                    <button type="button" data-modal-hide="edit-modal_{{ $job->id }}"
