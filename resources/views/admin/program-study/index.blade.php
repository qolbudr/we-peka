    ...
    <div class="p-4 mt-20 sm:p-6 lg:p-8">
        ...
        <div class="mb-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 sm:text-3xl">Program Studi</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Kelola daftar program studi</p>
                </div>
                                            ...
                                            <select name="university_id" required
                                                ...
                                                @foreach (\App\Models\University::all() as $univ)
                                                    <option value="{{ $univ->id }}"
                                                        {{ $prody->university_id == $univ->id ? 'selected' : '' }}>
                                                        {{ $univ->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                                ...
                                                <select name="level" required
                                                    ...
                                                    @foreach (\App\Enums\Level::cases() as $level)
                                                        <option value="{{ $level->value }}"
                                                            {{ $prody->level === $level->value ? 'selected' : '' }}>
                                                            {{ ucfirst($level->value) }}
                                                        </option>
                                                    @endforeach
                                                </select>
