<div id="edit-modal_{{ $quota->id }}" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative bg-white rounded-lg shadow-lg">
            ...
            <div class="flex items-center justify-between p-4 border-b border-gray-200 rounded-t md:p-5">
                ...
                <button type="button"
                    ...
                    data-modal-hide="edit-modal_{{ $quota->id }}">
            ...
            <!-- Modal body -->
            <form id="formEdit" action="{{ route('quota-mabas.update', $quota->id) }}" method="POST">
                @csrf
                ...
                <div class="p-4 space-y-4 md:p-5">
                    <!-- University Name -->
                    <div id="university-wrapper-{{ $quota->id }}">
                        <label for="university-id-{{ $quota->id }}"
                            class="block mb-2 text-sm font-medium text-gray-900">
                            University Name <span class="text-red-500">*</span>
                        </label>
                        <select name="university_id" id="university-id-{{ $quota->id }}" required
                            ...
                            @foreach ($universities as $univ)
                                <option value="{{ $univ->id }}" class="capitalize"
                                    {{ $univ->id == $quota->university_id ? 'selected' : '' }}>
                                    {{ $univ->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex items-center w-full gap-4">
                        ...
                        <div class="w-full">
                            ...
                            <input type="number" name="quota" id="quota" min="0" step="1"
                                value="{{ $quota->quota }}" oninput="validity.valid||(value='');"
                    ...
                    <div class="w-full">
                        ...
                        <input type="text" name="link" id="link" value="{{ $quota->link }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                    ...
                    <div>
                        ...
                        <textarea name="notes" id="notes" rows="4"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                            placeholder="Enter notes">{{ $quota->notes }}</textarea>
                    </div>
                </div>

                <!-- Modal footer -->
                <div class="flex items-center justify-end gap-3 p-4 border-t border-gray-200 rounded-b md:p-5">
                    <button type="button" data-modal-hide="edit-modal_{{ $quota->id }}"
