    ...
    <div class="p-4 mt-20 sm:p-6 lg:p-8">
        ...
        <div class="mb-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900 sm:text-3xl">Quota Management</h1>
                    <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Manage your quota</p>
                </div>
                <!-- Button Create -->
            ...
            {{-- Modal Create --}}
            @include('admin.quota-maba.create-modal')
        </div>

        <!-- Table Section -->
        <div class="relative overflow-auto rounded-lg shadow-md">
            @include('admin.quota-maba.table-quota')
        </div>
    </div>
@endsection
        ...
        $(document).ready(function() {
            ...
            $('#formCreate').on('submit', function(e) {
                if (!this.checkValidity()) {
                    ...
                    if (!this.quuniversity_idta.value) iziToast.error({
                        title: 'Error',
                        message: 'Universitas wajib diisi',
                    });

                    if (!this.year.value) iziToast.error({
                        title: 'Error',
                        message: 'Tahun wajib diisi',
                    });

                    if (!this.quota.value) iziToast.error({
                        title: 'Error',
                        message: 'Quota wajib diisi',
                    });

                    if (!this.link.value) iziToast.error({
                        title: 'Error',
