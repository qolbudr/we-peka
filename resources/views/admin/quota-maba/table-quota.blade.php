<table id="quiz-table" class="w-full">
    <thead class="text-xs text-white uppercase bg-blue-700">
        <tr>
            ...
            <th scope="col" class="px-4 py-4 text-start sm:px-6">University Name</th>
            <th scope="col" class="px-4 py-4 text-start sm:px-6">Year</th>
            <th scope="col" class="px-4 py-4 text-start sm:px-6">Quota</th>
            <th scope="col" class="px-4 py-4 text-start sm:px-6">Link</th>
            <th scope="col" class="px-4 py-4 text-start sm:px-6">Notes</th>
            <th scope="col" class="px-4 py-4 text-center sm:px-6">Actions</th>
        </tr>
    </thead>
    <tbody class="divide-y divide-gray-200">
        @forelse ($quotaMabas as $quota)
            <tr class="bg-white">
                <td class="px-4 py-4 font-medium text-gray-900 sm:px-6 whitespace-nowrap">
                    {{ $loop->iteration }}
                </td>
                <td class="px-4 py-4 text-gray-900 sm:px-6">
                    {{ $quota->university->name }}
                </td>
                ...
                <td class="max-w-xs px-4 py-4 text-gray-900 sm:px-6">
                    <a href="{{ $quota->link ?? '-' }}" target="_blank"
                        class="hover:underline hover:text-blue-600">Link</a>
                </td>
                <td class="max-w-xs px-4 py-4 text-gray-900 sm:px-6">
                    {{ $quota->notes ?? '-' }}
                </td>
                <td class="px-4 py-4 sm:px-6">
                    <div class="flex items-center justify-center gap-2">
                        <!-- Button Edit -->
                        <button data-modal-target="edit-modal_{{ $quota->id }}"
                            data-modal-toggle="edit-modal_{{ $quota->id }}"
                        ...
                        {{-- Modal Edit --}}
                        @include('admin.quota-maba.edit-modal')

                        <!-- Button Delete -->
                        <button type="button" data-url="{{ route('quota-mabas.destroy', $quota->id) }}"
            ...
            <tr>
                <td colspan="7" class="p-0">
                    <div
                        ...
                        <p class="text-lg font-semibold text-gray-700">No quota found</p>
                        <p class="mt-1 text-sm text-gray-500">Get started by creating your first quota.</p>
                    </div>
                </td>
            </tr>
        @endforelse
